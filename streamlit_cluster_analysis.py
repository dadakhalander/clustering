...
def load_model():
    try:
        ...
        st.error("Model file 'best_rf.pkl' not found. Please ensure it's in the correct directory.")
        st.stop()
...
def load_train_data():
    try:
        ...
        st.error("Training data file 'X_train.pkl' not found.")
        st.stop()
...
def load_cluster_info():
    try:
        ...
        st.error("Cluster info file 'cluster_k_info.pkl' not found.")
        st.stop()
...
def analyze_new_customer(new_data, model, X_train, cluster_info):
    try:
        ...
        st.error(f"An error occurred during customer analysis: {str(e)}")
...
def load_data():
    try:
        ...
        st.error("Data file 'clustering_results.csv' not found.")
        st.stop()
...
def main():
    ...
    if section == "📊 Cluster Analysis":
        cluster_analysis_section(df, model, X_train)
    elif section == "👤 Analyze New Customer":
        analyze_new_customer_section(model, X_train, cluster_k_info)
    elif section == "🔧 Custom Clustering":
        custom_clustering_section(df)
...
def cluster_analysis_section(df, model, X_train):
    ...
    df_filtered = df[
        ...
        (df['Spending_Score_original'] >= spending_range[0]) &
        (df['Spending_Score_original'] <= spending_range[1]) &
        (df['Loyalty_Score'] >= loyalty_range[0]) &
        (df['Loyalty_Score'] <= loyalty_range[1])
    ]
...
def analyze_new_customer_section(model, X_train, cluster_k_info):
...
def custom_clustering_section(df):
